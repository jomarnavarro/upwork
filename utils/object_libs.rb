...
module ObjectLibs
  ...
  def select_child(parent, descriptor)
    selector = element_selector(descriptor)
    parent.find_element(selector)
  end

  def select_children(parent, descriptor)
    selector = element_selector(descriptor)
    parent.find_elements(selector)
  end

  def select_elements(descriptor)
    selector = element_selector(descriptor)
    wait_until_present(selector)
  ...
  def wait_until_present(descriptor)
    selector = element_selector(descriptor)
    long_wait.until { @driver.find_element(selector).displayed? }
  end
  ...
  def child_exists?(parent, child_descriptor)
    child_selector = element_selector(child_descriptor)
    begin
      return true if parent.find_element(child_selector)
    rescue Selenium::WebDriver::Error::NoSuchElementError
      false
    end
  end
